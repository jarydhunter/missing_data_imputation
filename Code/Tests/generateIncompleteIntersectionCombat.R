...
# full data
generateIncompleteIntersectionCombat <- function(data) {
  ...
  subsetIDs <- colnames(subsetData[[1]])
  subsetIDs <- transformIDFormat(subsetIDs)
#   # Find the position of the patient IDs in the clinical data
#   clinicalIDs <- as.character(clinicalData$bcr_patient_barcode)
#   clinicalInd <- match(subsetIDs, clinicalIDs)
#   clinicalData <- clinicalData[clinicalInd, ]
#   # Subset clinical data and subsetData by clinicalData
#   clinicalData <- clinicalData[rowSums(is.na(clinicalData)) < ncol(clinicalData),]
#   clinicalIDs <- as.character(clinicalData$bcr_patient_barcode)
  subsetInd <- match(clinicalIDs, subsetIDs)
  
  ###### Run combat function 
  for (i in 1:numViews) {
    temp.subsetData <- subsetData[[i]]
    temp.subsetData <- temp.subsetData[,subsetInd]
    temp.2.subsetData <- temp.subsetData[rowSums(temp.subsetData) != 0,]
    temp.modcombat <- model.matrix(~1, data = clinicalData)
    temp.batch <- clinicalData$gender
    temp_combat = ComBat(dat=temp.2.subsetData, batch=temp.batch, mod=temp.modcombat, par.prior=TRUE, prior.plots=FALSE)
    subsetData[[i]] <- temp_combat
  }
