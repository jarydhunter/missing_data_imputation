...
evaluateClusteringCensor <- function(cluster, data, completeLabels,
  ...
  surv <- evaluateSurvivalCensor(clinicalData, labels)
  ...
  if (length(unique(intLabels))==1) {
    ...
    intSurv <- evaluateSurvivalCensor(intClinicalData, intLabels)
  }  
